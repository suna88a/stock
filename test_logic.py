...
def main():
    ...
    try:
        ...
        assert_equal(len(embeds), 4, 'daily report embed count')
        assert_equal(embeds[0].title, '📊 日次資産レポート', 'daily report title')
        summary_value = embeds[0].description
        ...
        assert_equal('基準 ' in summary_value, False, 'daily report omits base asset')
        assert_equal('目標 ' in summary_value, False, 'daily report omits target amount')
        assert_equal('資産内訳' in summary_value, False, 'daily report omits asset breakdown from first embed')
        assert_equal('更新情報' in summary_value, False, 'daily report omits update info from first embed')
        assert_equal('成果 🟢' in summary_value, True, 'daily report operation result icon')
        assert_equal('リスク概要' in summary_value, True, 'daily report risk summary')
        assert_equal('\nNET 下落影響' in summary_value, True, 'daily report separates decline section')
        assert_equal('最大 NET' in summary_value, True, 'daily report compact risk summary')
        assert_equal('🟡' in summary_value, True, 'daily report risk warning icon')
        assert_equal(embeds[1].title, '資産内訳・更新情報', 'daily report asset/update embed title')
        assert_equal('資産内訳' in embeds[1].description, True, 'daily report asset breakdown in second embed')
        assert_equal('更新情報' in embeds[1].description, True, 'daily report update info in second embed')
        holding_field = embeds[2].fields[0]
        holding_text = holding_field.value
        assert_equal(embeds[2].title, '保有銘柄', 'daily report holding embed title')
        assert_equal(holding_field.name, 'NET / Cloudflare', 'daily report holding display name')
        assert_equal(holding_text.startswith('```text'), True, 'daily report holding code block')
        assert_equal('前日 ' in holding_text, True, 'daily report holding day change row')
        assert_equal('+¥23,580' in holding_text, True, 'daily report holding day change jpy')
        assert_equal('円換算評価額' in holding_text, False, 'daily report omits jpy value duplicate')
        assert_equal('平均取得単価' in holding_text, False, 'daily report uses compact labels')
