...
def main():
    ...
    try:
        ...
        holding_text = embeds[1].description
        assert_equal(embeds[1].title, '保有銘柄', 'daily report holding embed title')
        assert_equal('NET / Cloudflare' in holding_text, True, 'daily report holding display name')
        assert_equal(holding_text.startswith('```text'), False, 'daily report holding avoids code block')
        ...
        assert_equal(embeds[2].title, '資産内訳・更新情報', 'daily report asset/update embed title')
        assert_equal('資産内訳' in embeds[2].description, True, 'daily report asset breakdown in third embed')
        assert_equal('更新情報' in embeds[2].description, True, 'daily report update info in third embed')
