...
def create_asset_embed(user_id, total_asset=None):
    ...
    if asset_info.get('last_updated'):
        embed.add_field(name="更新日時", value=str(asset_info['last_updated']), inline=False)
...
async def show_asset(interaction: discord.Interaction):
    """資産コマンド: /資産"""
    user_id = interaction.user.id
    await interaction.response.send_message(embed=create_asset_embed(user_id))
...
async def help_command(interaction: discord.Interaction):
    ...
    commands_info = [
        ...
        ("/資産", "現在資産・基準資産・入出金・資産内訳を表示します"),
        ("/資産更新 金額 通貨", "現在資産を手動更新します"),
